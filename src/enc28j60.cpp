...
uint8_t ENC28J60 ::Read_control_register(uint8_t reg) {
  ...
  // printf("read_commd %X\n", read_command);
  uint8_t data = read_byte(spi, reg_addrs, READ_CONTROL_REG);
  return data;
}
...
void ENC28J60::write_control_reg(uint8_t reg, uint8_t data) {
  ...
  uint8_t reg_addrs = get_reg_address(reg);
  switch_bank(reg_bank);
  uint16_t command = (WRITE_CONTROL_REG << OP_CODE_OFFSET | reg_addrs);
  command = (command << 8) | data;
  printf("write_command %X\n", command);
  ret = write_2_bytes(spi, command);
  if (ret != ESP_OK) {
    return;
  }
}
